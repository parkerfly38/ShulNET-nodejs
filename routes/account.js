...
function validateResetToken(req, res, next) {
    ...
    accountService.validateResetToken(req.body)
        .then(() => res.status(200).json({ message: 'Token is valid' }))
        .catch(next);
}
...
function resetPassword(req, res, next) {
    ...
    accountService.resetPassword(req.body)
        .then(() => res.status(200).json({ message: 'Password reset successful, you can now login' }))
        .catch(next);
}
...
function getById(req, res, next) {
    ...
    */
    ...
    accountService.getById(req.params.id)
        .then(account => account ? res.status(200).json(account) : res.sendStatus(404))
        .catch(next);
}
